...
(function(root, factory) {
    ...
    var Zoomage = function(options) {
        ...
        // add edge locked
        this.lockedge             = options.lockedge || false;
        ...
        if (this.enableGestureRotate) {
            ...
            // Adjust the scale of canvas ing surface
    ...
    Zoomage.prototype = {
            ...
            if ((this.isOnTouching || this.isFirstTimeLoad || this.mdown) && this.isImgLoaded) {
                ...
                if(this.isFirstTimeLoad){
                    if(currentWidth >= canvasWidth && currentHeight < canvasHeight){
                        this.imgState = 1 //宽撑满高不足
                    }else if( currentHeight >= canvasHeight && currentWidth < canvasWidth){
                        this.imgState = 2 //高撑满宽不足
                    }else if(currentWidth < canvasWidth && currentHeight < canvasHeight){
                        this.imgState = 3 //宽高均不足
                    }else{
                        this.imgState = 4 //宽高均超过
                    }
                }

                if(this.lockedge && this.imgState){

                    if(this.imgState == 1){
                        if(this.position.x >= 0){
                            this.position.x = 0;
                        }else if(this.position.x <= (canvasWidth - currentWidth)){
                            this.position.x = canvasWidth - currentWidth;
                        }
                        if(this.position.y <= 0){
                            this.position.y = 0;
                        }else if(this.position.y >= (canvasHeight - currentHeight)){
                            this.position.y = canvasHeight - currentHeight;
                        }
                    }
                    if(this.imgState == 2){
                        if(this.position.y >= 0){
                    ...
                    if(this.imgState == 4){
                        if(this.position.x < (canvasWidth - currentWidth)){
                            this.position.x = canvasWidth - currentWidth;
                        }else if(this.position.x > 0){
                            this.position.x = 0;
                        }
                        if(this.position.y < (canvasHeight - currentHeight)){
                            this.position.y = canvasHeight - currentHeight;
        ...
        _runCallback: function(flag, arg) {
            switch(flag) {
                ...
                case "onLoaded":
                if (this._type(this.onLoaded) === "function") {
            ...
            this.imgTexture.addEventListener('load', function(e) {
                ...
                if (this.imgTexture) {
                    ...
                    if (this.imgTexture.width > this.canvas.clientWidth) {
                       _widthScale = this.canvas.clientWidth / this.imgTexture.width
                       this.minZoom = Math.min(this.minZoom, _widthScale)
                    }
                    if (this.imgTexture.height > this.canvas.clientHeight) {
                        _heightScale = this.canvas.clientHeight / this.imgTexture.height
                         this.minZoom = Math.min(this.minZoom, _heightScale)
                    }
                }

                this._runCallback("onLoaded");
